...
- 플레이어 프로필 요약 API, 캐릭터 옵션/메타데이터 API, 매치 히스토리 표준 응답/더보기/필터/전체 갱신, DB 기준 match 조회, DB 기준 stats API 계산, streak/activity 확장 통계, locale 대응 가능 오류 코드, 추천 검색 API, 내부 플레이어 리더보드 API, stats/match/profile/search/leaderboard mapper-service 테스트, service 테스트, match sync/query 분리, 역할별 패키지 분리, DB match fresh 우선 사용/fallback은 1차 구현 완료 상태입니다.
- 리더보드와 검색 이력 기반 추천 검색의 API/DB/UI 확장 방향은 `docs/leaderboard-search-history.md`에 정리했습니다.
- 다음 단계는 프론트에 리더보드 UI를 연결하거나 Wavu 연동 경계를 설계하는 것입니다.

### 다음 작업

1. 프론트에서 내부 리더보드 API를 호출할 수 있게 API 클라이언트를 확장합니다.
2. Wavu 연동을 위한 `datasource.wavu` 패키지 경계를 설계합니다.
