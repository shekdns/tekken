...
- 플레이어 프로필 요약 API, 매치 히스토리 표준 응답/더보기/필터/강제 갱신, DB 기준 match 조회, stats API 필터/상세 지표, stats/match/profile mapper 테스트, service 테스트, match sync/query 분리, 역할별 패키지 분리, DB match fresh 우선 사용/fallback은 1차 구현 완료 상태입니다.
- 다음 단계는 저장 데이터 기반 통계 확장 범위를 설계하는 것입니다.

### 다음 작업

1. 저장된 `matches` 테이블 기반 통계 계산 범위를 설계합니다.
2. 상대 캐릭터별 승률과 캐릭터별 상세 전적을 프론트에서 활용할 수 있게 문서화합니다.
3. Wavu 연동을 위한 `datasource.wavu` 패키지 경계를 설계합니다.
